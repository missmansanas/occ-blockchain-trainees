...
contract Exchange is ERC20 {
  ...
  function getReserve() public view returns (uint256) {
    ...
    // Look up my custom token ("ATK")
    // Check how many ATKs this address (smart contract) is holding
  }

  /* add liquidity function 
  * Transfers ATK from the user -> this smart contract
 ...
 function addLiquidity (uint256 _amount) public payable returns (uint256) {
  ...
  uint tokenReserve = getReserve(); // ATK bal of this contract
  ERC20 token = ERC20(tokenAddress); // assigns ATK to token variable
 ...
 */
  ...
  function getAmountOfTokens(uint inputAmount, uint inputReserve, uint outputReserve)
    public pure returns (uint) { 
      ...
      uint256 denominator = inputReserve + inputAmountWithFee; // (x + Δx)
